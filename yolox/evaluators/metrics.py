...
def get_table(header, exp_table):
    table_header = header
...
class AttributeEval:
    def __init__(self, attr_num, conf=0.25, iou_thres=0.45):
        self.attr_num = attr_num  # number of attributes
        self.conf = conf
        self.iou_thres = iou_thres
        self.tps = 0
        self.errors = torch.zeros(self.attr_num)
    ...
    def process_batch(self, detections, labels):
        ...
        Arguments:
            detections (Array[N, 6 + attr_num]), x1, y1, x2, y2, conf, class, attrs
            labels (Array[M, 5 + attr_num]), class, x1, y1, x2, y2, attrs
        Returns:
            None, updates accordingly
        """ 
        detections = detections[detections[:, 4] > self.conf]
        gt_attrs = labels[:, 5:].int()
        dt_attrs = detections[:, 6:].int()
    ...
    def get_accurate(self):
        return (1 - self.errors / self.tps).numpy()
