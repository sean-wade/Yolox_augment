...
class Trainer:
    def __init__(self, exp: Exp, args):
        ...
        self.save_interval = exp.save_interval
    ...
    def evaluate_and_save_model(self):
        ...
        with adjust_status(evalmodel, training=False):
            val_res = self.exp.eval(
                evalmodel, self.evaluator, self.is_distributed
            )
            if len(val_res) == 3:
                ap50_95, ap50, summary = val_res
            else:
                ap50_95, ap50, summary, attr_acc = val_res
        ...
        if self.rank == 0:
            if self.args.logger == "tensorboard":
                ...
                if attr_acc is not None:
                    for i in range(attr_acc.shape[0]):
                        self.tblogger.add_scalar("val/attr_%d"%i, attr_acc[i], self.epoch + 1)
            ...
            if self.args.logger == "wandb":
                ...
                if attr_acc is not None:
                    self.wandb_logger.log_metrics({
                    "val/%d"%k:v for k,v in enumerate(attr_acc)
                })
            logger.info("\n" + summary)
        synchronize()

        self.save_ckpt("last_epoch", update_best_ckpt)
        if self.save_history_ckpt and (self.epoch+1) % self.save_interval==0:
