...
#             dataset_name="Fulda",
    ...
    def get_model(self):
        # Params: 7.59M, Gflops: 84.84
        from torch import nn
        from yolox.models import YOLOX, YOLOPAFPN, YOLOXHead, YOLOXHeadAttr, YOLOXFPN_234
        from yolox.data.datasets import DTLD_ATTRIBUTES
        ...
        if getattr(self, "model", None) is None:
            ...
            backbone = YOLOXFPN_234(self.depth, self.width, in_channels=in_channels, act=self.act)
            # head = YOLOXHead(self.num_classes, self.width, in_channels=in_channels, act=self.act)
